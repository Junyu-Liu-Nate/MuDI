...
class DreamBoothMultiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # num_tokens_to_use = len(current_tokens_list)
        ...
        # prompt = self.form_prompt_chair(tokens_ids_to_use, tokens_to_use)
        ...
        # # Save the instance image and masks for visualization
        ...
        # instance_image = inv_normalize(instance_image)
        # TF.to_pil_image(instance_image).save(instance_image_path)
...
class DreamBoothMultiSynthDataset(DreamBoothMultiDataset):
    """
    ...
    def __init__(
        ...
        self.instance_data_root = Path(instance_data_root)
        print(f'instance_data_root: {self.instance_data_root}')
        if not self.instance_data_root.exists():
            raise ValueError(f"Instance {self.instance_data_root} images root doesn't exists.")
        ...
        # print(f'self.instance_data_root.iterdir(): {self.instance_data_root.iterdir()}')
        self.instances = []
        for folder in sorted(self.instance_data_root.iterdir()):
            if folder.is_dir():
                instance_img_path = folder / "img.jpg"
        ...
        print(f'self._length: {self._length}')
    ...
    def uniform_sample_train_img(self, index, bg_token_idx):
        ...
        if self.randomize_unused_mask_areas:
            if self.use_bg_tokens:
                ...
                # raise ValueError("Invalid randomize_unused_mask_areas. Please set the background to white or use background tokens.")
        # print(f'instance_image size: {instance_image.size()}')
        # print(f'tokens_to_use: {tokens_to_use}')
        # self.visualize_instance_imgs(instance_image.squeeze(dim=0), instance_masks, index)
        # exit()
    ...
    def generate_concept_comb_across_train_imgs(self, bg_token_idx):
        ...
        if self.use_bg_tokens:
            ...
            # print(f'sample bg token: {bg_token}')

        # print(f'img_id_map: {img_id_map}')
        # print(f'synth_mask_ids: {synth_mask_ids}')
        ...
        # print(f'synth_prompt: {synth_prompt}')
        ...
        if self.synth_type == 'random-no-overlap':
            return img_id_map, synth_mask_ids, synth_tokens_to_use ### In this situation, we need to syntheize the image first and filter unused tokens
        else:
    ...
    def synthesize_test_img(self, tokens):
        ...
        ### TODO: May need to use other synthesize image methods - needs to ensure all contents are put onto the image
    ...
    def collate_fn(self, examples):
        ...
        # print(examples)
        # prompts = [examples["instance_prompt"], examples["synth_prompt"]]
        # prompts = [(ex["instance_prompt"], ex["synth_prompt"]) for ex in examples]
        prompts = [ex["synth_prompt"] for ex in examples] + [ex["instance_prompt"] for ex in examples]
    ...
    def __getitem__(self, index):
        ...
        if self.use_bg_tokens:
            ...
            bg_token_idx = -1
        # print(f'self.bg_placeholder_tokens: {self.bg_placeholder_tokens}')
        # print(f'bg_token_idx: {bg_token_idx}')
        # exit()
        ...
        if self.use_all_instance:
            ...
            synth_image, synth_masks, synth_tokens_ids_to_use_global, synth_prompt = self.uniform_sample_train_img(index)
        else:
            ...
            # print(f'synth_image shape: {synth_image.shape}')
            # print(f'synth_masks shape: {synth_masks.shape}')
            # self.visualize_synthsized_imgs(synth_image, synth_masks, index)
            # print(f"synth_tokens_ids_to_use_global: {synth_tokens_ids_to_use_global}")
            # print(f"synth_prompt: {synth_prompt}")
            # exit()
        ...
        #     return_tensors="pt",
        # ).input_ids
        example["synth_prompt"] = synth_prompt
        # print(f'synth_prompt: {synth_prompt}')
        # exit()
